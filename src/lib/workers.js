...
const workers = {
  ...
  processCheckOutcome: (originalCheckData, checkOutcome) => {
    ...
    const curAlertWarranted =
      originalCheckData.lastChecked && originalCheckData.state !== curState;
  ...
  alertUserToStatusChanged: newCheckData => {
    const curMessage = `Alert: Your check for ${newCheckData.method.toUpperCase()} ${
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, curMessage, err => {
      if (!err) {
        ...
        debug(
          'Error: Could not send sms alert to user who had a state change in there check'
        );
      }
    });
  },
